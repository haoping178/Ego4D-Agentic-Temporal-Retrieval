...
1. **Stage 1：候選秒檢索（Temporal Retrieval）**
2. **Stage 2：時間區間生成與重排（Temporal Proposal & Reranking）**
3. **Stage 3：VLM Agent 視覺驗證、邊界修正與最終重排**
...
影片以 **1 秒為單位**建立表示，每秒取樣 **8 Frames**，再利用：
...
1. 取得候選時間區間。
2. 從區間中依時間順序取樣 Keyframes。
3. 將 Query 與 Keyframes 一起交給 VLM Agent。
4. 判斷物件、動作、事件與 Query 是否一致。
5. 檢查事件是否完整。
6. 必要時進行保守的 temporal boundary refinement。
